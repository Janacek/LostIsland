...
void HarmlessAnimal::deserialize(std::ifstream &) throw (MyException)
{
	// load
}

HarmlessAnimal::HarmlessAnimal(sf::Vector2f &position, int life, Camera *cam)
...
HarmlessAnimal::~HarmlessAnimal()
{

}
void HarmlessAnimal::changeAnimation(sf::Vector2f&pos, std::pair<float, float>front)
{
...
void HarmlessAnimal::moveToNextWP()
{
	double dt = 0;
	double dt2 = 0;
	double time;
...
void HarmlessAnimal::doAction(AEntity *o)
{
	o->getAction(this);
}

void HarmlessAnimal::getAction(AEntity *o)
{
	_life -= o->getDamage();
	if (_life <= 0)
	{
...
void HarmlessAnimal::loadAnimation(std::string const &string_anim, float)
{
...
void HarmlessAnimal::update(Map &map)
{
...
Type HarmlessAnimal::getType() const
{
	return HARMLESS_ANIMAL;
}
sf::FloatRect HarmlessAnimal::getBoxCollider() const
{
